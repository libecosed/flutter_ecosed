...
class EcosedDashboardViewState extends State<EcosedDashboardView> {
  /// 占位用空模块
  ...
  Future<void> _initPlatformState() async {
    ...
    initialPluginList.add(widget);
    pluginDetailsList.add(
      PluginDetails(
        channel: widget.pluginChannel(),
        title: widget.pluginName(),
        description: widget.pluginDescription(),
        author: widget.pluginAuthor(),
        type: PluginType.flutter,
        initial: true,
      ),
    );
    ...
    if (Platform.isAndroid) {
      try {
        // 遍历原生插件
        for (var element
            in (await widget.onPlatformCall(getPluginMethod) as List? ??
                [_unknownPlugin])) {
          // 添加到插件详细信息列表
          pluginDetailsList.add(
